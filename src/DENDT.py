...
class dendt():
    def __init__(self,last_scan,new_scan,bounds,n = 50, reg = 0.0005, nn = 5, maxiter=1000):
        self.new_scan = new_scan
        self.Mus, self.Sigmas = self.ndt2(last_scan.T,n,reg, nn)
        self.result = differential_evolution(func, bounds,maxiter=1000)
        self.T = self.result.x
        
    def ndt2(self,X,n = 50, reg = 0.0005, nn = 5):
        idxs = np.random.choice(len(X),n)
        self.Mus = X[idxs]
        nbrs = NearestNeighbors(n_neighbors=nn, algorithm='ball_tree').fit(Mus)
        self.Sigmas = []
        for ii in range(n):
            idxs = nbrs.kneighbors(self.Mus[ii].reshape((1,2)),n_neighbors = nn, return_distance=False)
            c = np.cov(Mus[idxs].reshape(Mus[idxs].shape[1],Mus[idxs].shape[2]).T) + reg*np.eye(2)
            self.Sigmas.append(c)
...
if __name__ == "__main__":
    ...
    Dendt = dendt(last_scan=last_scan,new_scan=new_scan,bounds=bounds)
    new_scan_T = np.array(transform(new_scan.T,Dendt.T).T)
